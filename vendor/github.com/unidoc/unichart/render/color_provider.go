...
func Jet(v, vmin, vmax float64) color.Color {
	...
	if v < (vmin + 0.25*dv) {
		c.R = 0
		c.G = uint8((4 * (v - vmin) / dv) * 255)
	} else if v < (vmin + 0.5*dv) {
		c.R = 0
		c.B = uint8((1 + 4*(vmin+0.25*dv-v)/dv) * 255)
	} else if v < (vmin + 0.75*dv) {
		c.R = uint8((4 * (v - vmin - 0.5*dv) / dv) * 255)
		c.B = 0
	} else {
		c.G = uint8((1 + 4*(vmin+0.75*dv-v)/dv) * 255)
		c.B = 0
	}
...
var viridisColors = [256]color.RGBA{
