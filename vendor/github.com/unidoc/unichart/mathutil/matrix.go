...
func (m *Matrix) Pivotize() *Matrix {
	...
	p := ZeroMatrix(m.stride, m.stride)
	for r, c := range pv {
...
func (m *Matrix) LU() (l, u, p *Matrix) {
	l = ZeroMatrix(m.stride, m.stride)
	u = ZeroMatrix(m.stride, m.stride)
	p = m.Pivotize()
	m, _ = p.Multiply(m)
...
func (m *Matrix) Inverse() (*Matrix, error) {
	...
	aug, _ := m.Augment(EyeMatrix(rows))
	for i := 0; i < rows; i++ {
		j := i
		for k := i; k < rows; k++ {
